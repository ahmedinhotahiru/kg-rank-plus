...
def get_umls_keys(query, answer, prompt, llm):
    ...
    try:
        ...
        # Create expanded query with the answer
    ...
    for key in entities:
        ...
        while queue:
            ...
            if len(relations) > max_to_expand:
                ...
                for rel in relations:
                    ...
                    relation_texts.append(f"{related_from} {relation_label} {related_to}")
        ...
        if relation_texts:
            ...
            #         # Keep top indices for this hop level
            #         filtered_indices.extend([idx for idx, _ in rel_infos[:max_to_keep]])
            #     else:
            #         # Keep all for this hop level
            ...
            # ----------------- NEW APPROACH -----------------
            # Organize by hop level, keeping track of the original indices
            relations_by_hop = {1: [], 2: [], 3: []}
            for i, rel_info in enumerate(all_relations):
                hop_level = rel_info[0]
                relation_text = rel_info[3]  # This is the relation text
                
                # Store tuple of (original_index, relation_text)
                if hop_level in relations_by_hop:
                    relations_by_hop[hop_level].append((i, relation_text))

            # Filter and select by relevance within each hop level
            filtered_relation_indices = []

            for hop_level, hop_relations in relations_by_hop.items():
                if not hop_relations:
                    continue
                    
                indices, texts = zip(*hop_relations) if hop_relations else ([], [])
                
                # Get embeddings for all relations at this hop level
                if texts:
                    embeddings = umlsbert.batch_encode(list(texts))
                    
                    # Compute similarity scores
                    scores = []
                    for i, embedding in enumerate(embeddings):
                        sim = float(get_similarity([expanded_embedding], [embedding])[0][0])
                        scores.append((indices[i], sim))
                    ...
                    max_to_keep = max_relations_to_keep.get(hop_level, 20)
                    top_indices = [idx for idx, _ in scores[:max_to_keep]]
                    filtered_relation_indices.extend(top_indices)

            # Now get the actual filtered relations
            filtered_relations = [all_relations[i] for i in filtered_relation_indices]

            # Get the embeddings for these filtered relations
            filtered_texts = [rel[3] for rel in filtered_relations]  # Use the text directly from all_relations
            ...
            for i, (hop_level, full_path, _, _) in enumerate(filtered_relations):
                ...
                # Apply hop discount to favor direct relations
                hop_discount = 0.95 ** (hop_level - 1)
            ...
            for idx, _ in top_relations:
                hop_level, full_path, _, _ = all_relations[idx]
