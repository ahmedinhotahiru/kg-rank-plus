...
def get_umls_keys(query, prompt, llm):
    ...
    max_hops = 3  # Reduced from 3 to 2 for performance
    max_relations_per_level = {
        1: 20,  # Level 1: Expand top 15 concepts (reduced from 20)
        2: 7,   # Level 2: Expand top 5 concepts (reduced from 7)
        3: 3    # Add back level 3
    }
    max_relations_to_keep = {
        1: 100,  # Keep top 50 1-hop relations (reduced from 100)
        2: 50,  # Keep top 25 2-hop relations (reduced from 50)
        3: 20   # Keep top 10 3-hop relations (reduced from 20)
    }
    entities = entities  # Remove the [:3] limit to process all entities
 # Maximum final relations to return (reduced from 20)
    ...
    for key in entities:
        ...
        while queue:
            ...
            # Limit relations based on hop level without prioritization
        ...
        if all_relations:
            # Get all relation texts
            relation_texts = [rel_info[3] for rel_info in all_relations]
            ...
            # Filter top relations by relevance for each hop level
            filtered_indices = []
